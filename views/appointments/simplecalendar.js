var calendar = {
    ...
    init: function(ajax) {
        ...
        if (ajax) {
            ...
            $.ajax({
                    url: 'views/appointments/events.json',
                    type: 'GET',
                })
                .done(function(data) {
                    ...
                    for (var i = 0; i < events.length; i++) {
                        $('.list-app').append('<div class="day-event-app" date-day="' + events[i].day + '" date-month="' + events[i].month + '" date-year="' + events[i].year + '" data-number="' + i + '"><a href="#" class="close fontawesome-remove"></a><h2 class="title">' + events[i].title + '</h2><p>' + events[i].description + '</p><label class="check-btn"><input type="checkbox" class="save" id="save" name="" value=""/><span>Save to personal list!</span></label></div>');
                    }
                ...
                .fail(function(data) {
                    console.log("I failed loading the datA1");
                    console.log(data);
                });
        } else {
    ...
    startCalendar: function() {
        ...
        $('.btn-next-app').on('click', function(e) {
        ...
        $('.btn-prev-app').on('click', function(e) {
            var monthNumber = $('.month').attr('data-month');
            if (monthNumber < 2) {
        ...
        /**
         * Get current day and set as '.current-day-app'
         */
        function setCurrentDay(month, year) {
            ...
            if (parseInt(year) === yearNumber) {
                if (parseInt(month) === parseInt(viewMonth)) {
                    $('tbody.event-calendar td[date-day="' + d.getDate() + '"]').addClass('current-day-app');
                }
            }
        };
         ...
         */
        function setEvent() {
            $('.day-event-app').each(function(i) {
                var eventMonth = $(this).attr('date-month');
                var eventDay = $(this).attr('date-day');
                var eventYear = $(this).attr('date-year');
                var eventClass = $(this).attr('event-class');
                if (eventClass === undefined) eventClass = 'event-app';
                else eventClass = 'event-app' + eventClass;
         ...
         * and find day-event-app to display
         */
        ...
        $('.save').click(function() {
            if (this.checked) {
                ...
                var eventHtml = $(this).closest('.day-event-app').html();
                var eventMonth = $(this).closest('.day-event-app').attr('date-month');
                var eventDay = $(this).closest('.day-event-app').attr('date-day');
                var eventNumber = $(this).closest('.day-event-app').attr('data-number');
        ...
        function remove() {
            $('.remove').click(function() {
                if (this.checked) {
                    ...
                    $('.day[date-month="' + eventMonth + '"][date-day="' + eventDay + '"][data-number="' + eventNumber + '"]').slideUp('slow');
                    $('.day-event-app[date-month="' + eventMonth + '"][date-day="' + eventDay + '"][data-number="' + eventNumber + '"]').find('.save').attr('checked', false);
                    $('.day-event-app[date-month="' + eventMonth + '"][date-day="' + eventDay + '"][data-number="' + eventNumber + '"]').find('span').text('Save to personal list');
                    setTimeout(function() {
                        $('.day[date-month="' + eventMonth + '"][date-day="' + eventDay + '"][data-number="' + eventNumber + '"]').remove();
                    }, 1500);
                }
